...
def scrap_profile(ids):
    ...
    for id in ids:
        ...
        user_folder = os.path.join(folder, id.split('/')[-1])

        try:
            if not os.path.exists(user_folder):
                os.mkdir(user_folder)
            else:
                print("A folder with the same profile name already exists. I will Remove it. Removing " + user_folder)
...
def main():
    ...
    # password is argument 2
    password = sys.argv[2]

    # action:login or scrap is argument 3
    action = sys.argv[3]
    ...
    if action=='login':
        ...
        # fetch my profile url where all personal data are located
        global driver
        ids = [driver.find_element_by_class_name('_606w').get_attribute('href') + '\n'];
        print(ids);
